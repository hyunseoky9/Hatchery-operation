classdef Env2_0 < handle
    ...
    methods
        function obj = Env2_0(initstate, parameterization_set, discretization_set)
            ...
            obj.parset      = parameterization_set - 1; % if you still want to store (set - 1)
        ...
        function [reward, done] = step(obj, action)
            ...
            if NW > obj.Nth  
                ...
                if tau == 0 
                    ...
                    try
                        % Poisson draw
                        lambdaPoisson = poissrnd(recruitment);  %#ok<*PFBNS> 
                        % Then binomial
                        NW_nextVal = binornd(lambdaPoisson, s);
                    catch
                        % If something goes wrong with the large Poisson, fallback to normal
                        muNormal = recruitment;
                        sigmaNormal = sqrt(recruitment);
                        normalDraw = round(normrnd(muNormal, sigmaNormal));
                        if normalDraw < 0
                            normalDraw = 0;  % no negative draws
                        end
                        NW_nextVal = binornd(normalDraw, s);
                    end
                    % 2) NWm1_nextVal
                ...
                if tau == 0
                    ...
                    NH_nextVal = aVal;  % as in your code
                ...
                nextState = [ NW_next_idx, NWm1_next_idx, NH_nextVal, H_next_idx, q_next_idx, tau_next ];
                nextObs   = [ y_next_idx,  NH_nextVal,    H_next_idx, q_next_idx, tau_next ];
        ...
        function val = discretize(obj, x, possibleStates)
            ...
            wUpper = 1 - wLower;                 % Probability of picking the upper bin
